...
def writeVideo(humanseg, bg_im, fps, size):
    ...
    # 将每一帧设置背景
    files = [humanseg + i for i in files]
    # print(files)
    for file in files:
...
def changeVideoScene(video_name, bgname):
    ...
    # 读取视频中每一帧画面
    fps, size = getFrame(video_name, frame_path)  # 这种过程的结果最好不要依赖上面函数得到的参数
...
if __name__ == '__main__':
    ...
    # 最终视频的保存路径
    output_video = BASE_DIR + '/5-3re.mp4'

    if not os.path.exists(frame_path):
        os.makedirs(frame_path)

    try:
        changeVideoScene('5-3.mp4', 'bg.jpg')
        print('你的视频已经制作完成')
